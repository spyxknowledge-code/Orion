...
// ---------- DATABASE PATH (local file, no /data) ----------
const DB_PATH = process.env.DB_PATH || './orionchat.db';
...
app.post('/create_room', (req, res) => {
  ...
  let decoded;
  try { decoded = jwt.verify(token, process.env.JWT_SECRET || 'orionsecret'); } catch (e) { return res.status(401).json({ error: 'Invalid token' }); }
