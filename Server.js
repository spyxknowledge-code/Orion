...
// ---------- DATABASE (persistent disk at /data) ----------
const DB_PATH = process.env.DB_PATH || '/data/orionchat.db';
...
app.post('/register', (req, res) => {
  ...
  try {
    insert.run(username, email, hash);
  } catch (e) {
    return res.status(400).json({ error: 'Username or email already taken' });
  }
  const token = jwt.sign({ username }, process.env.JWT_SECRET || 'orionsecret', { expiresIn: '7d' });
...
app.post('/create_room', (req, res) => {
  ...
  let decoded;
  try {
    decoded = jwt.verify(token, process.env.JWT_SECRET || 'orionsecret');
  } catch (e) {
...
io.use((socket, next) => {
  ...
  try {
    const decoded = jwt.verify(token, process.env.JWT_SECRET || 'orionsecret');
...
io.on('connection', (socket) => {
  ...
  const stmt = db.prepare(
    'SELECT id, sender, encrypted_text, iv, timestamp, edited, read_by FROM messages WHERE room_id = ? ORDER BY timestamp DESC LIMIT 50'
  );
  ...
  socket.on('send-message', (data) => {
    ...
    const insert = db.prepare(
      'INSERT INTO messages (room_id, sender, encrypted_text, iv, read_by) VALUES (?, ?, ?, ?, ?)'
    );
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`Orion Private Chat running on port ${PORT}`);
});
